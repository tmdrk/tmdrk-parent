...
public class JedisUtil {
    ...
    public static boolean setBit(String key,long offset,boolean bool){
        if (isValueNull(key, offset)) {
            return false;
        }
        Jedis jedis = null;
        try {
            jedis = JedisPoolUtil.getJedis();
            return jedis.setbit(key,offset,bool);
        }finally {
            JedisPoolUtil.closeJedis(jedis);
        }
    }
    ...
    public static long bitpos(String key,boolean bool){
        if (isValueNull(key,bool)) {
            return -1;
        }
        Jedis jedis = null;
        try {
            jedis = JedisPoolUtil.getJedis();
            return jedis.bitpos(key,bool);
        } finally {
            JedisPoolUtil.closeJedis(jedis);
        }
    }
    ...
    public static long bitOp(BitOP op,String destKey, String... srcKeys){
        if (isValueNull(op,destKey,srcKeys)) {
            return -1;
        }
        Jedis jedis = null;
        try {
     ...
     * @return
     */
    public static <K, V> int setMap(String key, Map<String, V> value) {
     ...
     * @return
     */
    public static <K, V> Map<String, V> getMap(String key, Class<V> v) {
        ...
        try {
            ...
            Map<String, V> result = (Map<String, V>) JSON.parseObject(data);
            return result;
        } finally {
            JedisPoolUtil.closeJedis(jedis);
        }
