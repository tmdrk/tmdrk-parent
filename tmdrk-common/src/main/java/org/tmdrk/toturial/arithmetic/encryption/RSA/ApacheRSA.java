...
public class ApacheRSA {
    ...
    public static void main(String[] args)throws Exception{
        String mMsg = "skhflskdflskdjflskdjfslkdjflskdfhslkdhfl而烈日光辉雷人化工i而化工欸如果黑skdhfslkdfhsldkfjsldkfjslkdfjslkdjflskdffffffjlkjwliehfowiehfwiuefh时刻提防还是老地方还是老地方驾驶的飞机看i恶化覅无恶虎覅无恶虎覅五俄方欧委会佛为哈佛违法和我i俄方烈火仏我iehfowiefhnowihfyhthyhf";
        Map<String, Key> keyMap = initKey();
        System.out.println(RSA_PUBLIC_KEY+":"+getPublicKey(keyMap));
        System.out.println(RSA_PRIVATE_KEY+":"+getPrivateKey(keyMap));
        byte[] bytes = encryptByPublicKey(mMsg.getBytes(), getPublicKey(keyMap).getEncoded());
        byte[] bytes1 = decryptByPrivateKey(bytes, getPrivateKey(keyMap).getEncoded());
        ByteArrayOutputStream bao = new ByteArrayOutputStream();
        bao.write(bytes);
        String ciphertext = new String(java.util.Base64.getEncoder().encode(bao.toByteArray()));
        System.out.println("ciphertext:"+ciphertext);
        System.out.println("plaintext:"+new String(bytes1));
    }
}
